...
void ReadPowerOnState() {
  bool tmpIsPowerOn = (digitalRead(solarPumpPowerOnPin) == LOW);
  if (tmpIsPowerOn != _solarPumpPowerOn) {
    _solarPumpPowerOn = tmpIsPowerOn;
  ...
  if (tmpIsPowerOn != _systemPowerOn) {
    _systemPowerOn = tmpIsPowerOn;
...
// water pump is enabled always and controlled by pressure switch
  tmpIsPowerOn = (digitalRead(waterCutOffPin) == LOW);
